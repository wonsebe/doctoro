...
//     pageInfo.categoryno=categoryno; console.log('카테고리 변경'); console.log(pageInfo);
//     print(1) ; //새로 고침, 1페이지
    ...
    $.ajax({
     ...
     //4. 페이지네이션 ( 페이지 버튼 ) 구성
        ...
        for( let current = 1 ; current <= totalPage ; current++ ){
            pageHTML += `<li class="page-item"><a class="page-link" onclick="print(${current})">${current}</a></li>`;
        }
        // 다음버튼 , page : 현재 함수의 매개변수 이면서 현재페이지번호 의미 , 현재페이지 + 1 , 만약에 현재페이지+1 했을때 최대페이지수 보다 커지면 최대페이지수 고정 아니면 +1 증가
        pageHTML += `<li class="page-item">
                        <a class="page-link" onclick="print( ${ page+1 > totalPage ? totalPage : page+1 } )">다음</a>
                    </li>`;
